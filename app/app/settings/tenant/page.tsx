...
export default async function TenantSettingsPage() {
  ...
  const lostReasonsExtra =
    (row?.settings && Array.isArray((row.settings as { lost_reasons_extra?: unknown }).lost_reasons_extra)
      ...
      : []) as string[];
  const colaDoMercado =
    typeof (row?.settings as { cola_do_mercado?: unknown } | null)?.cola_do_mercado === "string"
      ? ((row?.settings as { cola_do_mercado: string }).cola_do_mercado)
      : "";
