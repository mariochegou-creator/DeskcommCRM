...
-- ---- métricas de prospecção: fn_prospecting_metrics (migration 0093) ----
-- Agregação da seção "// prospecção" do Painel: entradas por etapa (fluxo, nos
-- DOIS dialetos de payload de stage_changed + leads nascidos na etapa), taxa de
-- resposta por contato (outbound da IA CONTA — cadência anti-vácuo é
-- prospecção), enrollments vivos por dia de cadência e "no vácuo" (último
-- enrollment 'exhausted' sem enrollment vivo). SECURITY INVOKER — a RLS escopa.
...
-- ---- tarefas pontuais do plano de vendas: plan_tasks (migration 0094) ----
-- Só o marcável mora aqui; rotina diária e checkpoints são constantes de
-- código (lib/plan/sixty-day-plan.ts). Unique (org, plan_key, slug) é o alvo
-- do upsert do seed. Fora do realtime: React Query invalida na mutação.
...
-- ---- gaveta de audios prontos do composer: saved_audios (migration 0095) ----
-- Escopo espelha message_templates (0060): owner_user_id null = compartilhado
-- da org. O binario mora no bucket whatsapp-media sob {org}/library/ — o envio
-- copia pra pasta da conversa (saved-audios/[id]/attach) pra nao afrouxar a
-- checagem de posse isMediaPathOwnedBy.
...
-- ===========================================================================
-- 0106 — copiloto da ligação (colunas novas + cascata LGPD atualizada).
-- Apêndice ao baseline pelo mesmo motivo da 0101: quem instala do zero aplica
-- este arquivo, não a cadeia de migrations.
-- ===========================================================================
...
-- ---- disparador em massa: broadcasts + broadcast_recipients (migration 0108) ----
-- Fila, trava de duplicata e relatorio na MESMA linha; claim atomico com lease.
-- O racional completo esta no cabecalho de supabase/migrations/20260824160000_0108_disparador.sql
